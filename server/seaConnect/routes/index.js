...
let boatMode= require('../model/boat_Model')
...
router.get('/getRoute',async(req,res)=>{
  try {
    let Route = await RouteModel.find()
    res.json(Route)
  } catch (error) {
    console.log(error)
      res.json(false)
...
router.post('/searchByBus',async(req,res)=>{
      try {
              console.log(req.body)
              const name = req.body.busName;
             let busResult = await jurneyModel.find({Boatname:name})
